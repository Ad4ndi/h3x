...
void print_hex
(FILE *f, int cols, int uppercase, int groups, int postscript, size_t len) {
    unsigned char buffer[DEF_COLS];
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "huc:rg:vl:p")) != -1) {
        switch (opt) {
            ...
            case 'r': rev = 1; break;
            case 'g': groups = strtol(optarg, NULL, 10); break;
            case 'v': printf("h3x v1.3 by Ad4ndi\n"); exit(0);
    ...
    if (rev) {
        ...
        print_hex(f, cols, upcase, groups, postscript, len);
    }
